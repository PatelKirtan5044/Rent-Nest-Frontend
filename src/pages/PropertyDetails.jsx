...
const PropertyDetails = () => {
  ...
  const handleApplyProperty = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await apiFetch(`/properties/${id}/apply`, {
        method: 'POST',
        body: JSON.stringify(applicationForm)
      });
      if (res.success) {
        addToast('Rental application submitted successfully!', 'success');
        setShowApplyModal(false);
        setApplicationForm({ incomeDetails: '', creditScore: '700', leaseTermMonths: '12', moveInDate: '', message: '' });
      }
    } catch (err) {
      addToast(err.message || 'Failed to submit application.', 'danger');
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleShortlist = () => {
    ...
    addToast(shortlisted ? 'Listing removed from shortlists.' : 'Listing added to shortlists!', 'success');
  };
    ...
    return (
      <div className="page-container" style={{ alignItems: 'center', justifyContent: 'center' }}>
        <div style={styles.errorCard}>
          ...
          <p>We could not find the property details you requested.</p>
          <button onClick={() => navigate('/properties')} className="btn btn-primary" style={{ marginTop: '20px' }}>
            Back to Listings
          </button>
        </div>
      </div>
  ...
  return (
    <div className="page-container" style={{ gap: '24px' }}>
      <button onClick={() => navigate('/properties')} style={styles.backBtn} className="btn btn-secondary">
        <ArrowLeft size={16} />
          ...
          <div style={styles.headerRight} className="details-header-right">
            {user.role === 'tenant' && (
              property.status === 'available' ? (
                <button onClick={() => setShowApplyModal(true)} className="btn btn-primary" style={styles.contactBtn}>
                  Contact Owner
                </button>
              ) : (
            ...
            <button onClick={toggleShortlist} className="btn btn-secondary" style={styles.shortlistBtn}>
              <Heart size={16} fill={shortlisted ? 'var(--danger)' : 'none'} color={shortlisted ? 'var(--danger)' : 'var(--text-secondary)'} />
            ...
            <div style={styles.ownerDetailsContainer} className="glass-panel">
              <div style={styles.ownerHeader}>
                ...
                <div>
                  ...
                  <span style={styles.ownerBadge}>Primary Contact / Landlord</span>
                </div>
              </div>
              <div style={styles.ownerInfoList}>
                <div style={styles.ownerInfoItem}>
                  <div style={styles.ownerInfoLabel}>
                    <Mail size={16} color="var(--primary)" />
                    <span>Email Address</span>
                  </div>
                  <strong style={styles.ownerInfoVal}>{property.landlord?.email || 'N/A'}</strong>
                </div>
                <div style={styles.ownerInfoItem}>
                  <div style={styles.ownerInfoLabel}>
                  ...
                  <strong style={styles.ownerInfoVal}>{property.landlord?.contactNumber || 'N/A'}</strong>
                </div>
              </div>
            </div>
          )}
      ...
      {/* --- MODAL: RENTAL APPLICATION FORM (Tenant only) --- */}
      {showApplyModal && (
        <div className="modal-overlay" onClick={() => setShowApplyModal(false)}>
          <div className="modal-content modal-card" onClick={(e) => e.stopPropagation()} style={styles.applyModal}>
            <div style={styles.applyModalHeader}>
              <h2>Rental Application</h2>
              <button onClick={() => setShowApplyModal(false)} style={styles.closeBtnNew}>
                <X size={20} />
              </button>
            </div>

            <form onSubmit={handleApplyProperty} style={styles.applyForm}>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setApplicationForm({ ...applicationForm, incomeDetails: e.target.value })}
              ...
              <div className="form-group">
                ...
                <select
                  value={applicationForm.leaseTermMonths}
                  onChange={(e) => setApplicationForm({ ...applicationForm, leaseTermMonths: e.target.value })}
